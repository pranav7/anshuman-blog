...
class ClfCallback(Callback):
    def __init__(self, learner, bs, xtraindata, xtestdata, ytraindata, ytestdata):
        super().__init__(learner)
        self.accuracies = []
        self.val_accuracies = []
        self.bs = bs
    ...
    def epoch_end(self):
        ...
        self.ytrainprob = self.learner.model(self.xtraindata)
        self.ytrainpredic = 1*(self.ytrainprob > 0.5)
        
       
        
        accuracy_train = np.mean(self.ytrainpredic == self.ytraindata)
        self.accuracies.append(accuracy_train)
        
        
        self.ytestprob = self.learner.model(self.xtestdata)
        self.ytestpredic = 1*(self.ytestprob > 0.5)
        
        
        accuracy_test = np.mean(self.ytestpredic == self.ytestdata)
        self.val_accuracies.append(accuracy_test)
        
        
        
        if self.epoch % 10 ==0:
            print(f"Epoch {self.epoch} Loss {eloss}")
            print("train accuracy is: {}, test accuracy is {}".format(accuracy_train, accuracy_test))
